...
;(function () {
      ...
      <div class="wrap">
        ...
        <div class="body">
          ...
          <div class="suggestions" id="suggestions">
            ...
            <div class="suggest-chip-row" id="suggestChipRow"></div>
          </div>
    ...
    const TRY_ASKING_BANK = [
      'What was the Monthly Active Users (MAU) for the last month?',
      'How has MAU trended over the past 6 months?',
      'What is the SVOD MAU for the last month?',
      'What is the AVOD MAU for the last month?',
      'How do SVOD MAU and AVOD MAU compare over the past 6 months?',
      'Which month recorded the highest MAU in the dataset?',
      'What is the Daily Active Users (DAU) for the most recent month?',
      'How does DAU compare between SVOD and AVOD users?',
      'What is the average number of active days per MAU (Avg. MAU Days)?',
      'Which month had the highest average MAU days?',
      'What was the Monthly Active Viewers (MAV) last month?',
      'How has MAV trended over the past 6 months?',
      'What is the SVOD MAV for the last month?',
      'What is the AVOD MAV for the last month?',
      'How do SVOD MAV and AVOD MAV compare month-over-month?',
      'What is the average daily active viewers (DAV) for the current month?',
      'Compare SVOD DAV and AVOD DAV for the past 3 months.',
      'What is the average number of active days per viewer (Avg. MAV Days)?',
      'Which month recorded the highest MAV?',
      'What is the monthly user-to-viewer conversion rate?',
      'How has user-to-viewer conversion changed over the last 6 months?',
      'What is the daily user-to-viewer conversion rate?',
      'Which month had the highest user-to-viewer conversion?',
      'How does conversion vary month-over-month?',
      'What were the total video views last month?',
      'What were the SVOD video views last month?',
      'What were the AVOD video views last month?',
      'How have video views trended over the past 6 months?',
      'Which month recorded the highest number of video views?',
      'How do SVOD video views compare with AVOD video views over time?',
      'What was the total watch time last month?',
      'What was the SVOD total watch time last month?',
      'What was the AVOD total watch time last month?',
      'How has watch time trended over the last 6 months?',
      'What is the average watch time per viewer (WT/Viewer)?',
      'How does WT/Viewer compare between SVOD and AVOD viewers?',
      'Which month recorded the highest watch time per viewer?',
      'What were the total monetizable views last month?',
      'What were the SVOD monetizable views last month?',
      ...
      'What are the Top 5 shows based on watch time?',
      'What are the Top 5 shows based on watch time per viewer?',
      'Show the Top 5 SVOD shows based on watch time.',
      'Show the Top 5 AVOD shows based on video views.',
      'Show MAV performance by content category for the last month.',
    ...
    class PerciBot extends HTMLElement {
      ...
      _renderTryAskingChips () {
        const row = this.$('suggestChipRow')
        if (!row) return
        const copy = TRY_ASKING_BANK.slice()
        for (let i = copy.length - 1; i > 0; i--) {
          const j = Math.floor(Math.random() * (i + 1));
          [copy[i], copy[j]] = [copy[j], copy[i]]
        }
        const six = copy.slice(0, 6)
        row.innerHTML = ''
        six.forEach(q => {
